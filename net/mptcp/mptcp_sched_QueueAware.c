...
static u32 sampleTime[10];
static u32 compBytes[10];
static long queueTime[10];
...
bool mptcp_is_available(struct sock *sk, const struct sk_buff *skb,
			bool zero_wnd_test)
{
    //get_interface_name(sk);
        
    return !mptcp_is_def_unavailable(sk) &&
	       !mptcp_is_temp_unavailable(sk, skb, zero_wnd_test); // Check if MPTCP connection is established and there is space in the send buffer
}
...
//*** Code for BQL functions start here ****///

/*  Trial code for getting queue disc values
void get_interface_name(struct sock *sk){
    ...
    //printk(KERN_INFO "Backlog in queue %u\n", mac80211_get_backlog(dst->dev));
    
    printk(KERN_INFO "MPTCP: The interface name is %s\n", dst->dev->name);
    printk(KERN_INFO "MPTCP: Total packets in queue are %u and total completed are %u\n", dql0->num_queued, dql0->num_completed);

    printk(KERN_INFO "====== QUEUE FROM QDISC ========");
    printk(KERN_INFO "Current packets in queue %u\n", qdisc0->qstats.qlen);
    printk(KERN_INFO "Packets sent: %u; Drops: %u; Requeues: %u; Backlog: %u\n", qdisc0->bstats.packets, qdisc0->qstats.drops, qdisc0->qstats.requeues, qdisc0->qstats.backlog);

    return;
}*/

long weighted_wait_average(long currentTime, long prevTime){
    long average;
    long average1;

    //alpha is chosen as 0.2
    average1 = (2 * currentTime);
    average1 = average1 / 10;
    average = prevTime + average1;

    //printk(KERN_INFO "Previous: %li, New: %li, with alpha: %li, average: %li", prevTime, currentTime, average1, average);
    
    return average;
}

u32 get_queue_size(struct sock *sk){
     struct dst_entry *dst;
     struct netdev_queue *txq0;
     struct dql *dql0;
     ...
     //printk(KERN_INFO "Packets in queue %u, path index: %u", packets_in_queue, tcp_sk(sk)->mptcp->path_index);

     return packets_in_queue;
}


long get_wait_time(struct sock *sk){
    struct timespec ts;
    long wait_time;
    //u32 rtt = 0xffffffff;
    struct dst_entry *dst;
    ...
    //get time of taking this sample
    getnstimeofday(&ts); // get current time
    currentTime = ts.tv_nsec; // / 1000; // store in microsecs same as RTT
    prevTime = sampleTime[tp->mptcp->path_index -1];
    samplingTime = currentTime - prevTime;

    packets_in_queue = dql0->num_queued - dql0->num_completed; //number of packets in DQL
    
    //get bytes that have been completed by dql
    completedBytes = dql0->num_completed;
    prevBytes = compBytes[tp->mptcp->path_index - 1];
    dequeuedBytes = completedBytes - prevBytes;


    if (packets_in_queue > 0 && dequeuedBytes > 0){

        /*If the completed bytes are less than
         * previous complted bytes or is 0, then
         * set time in queue as 0.
        ...
        // take the average of previous timeInQueue wih new one
        timeInQueue = weighted_wait_average(timeInQueue, queueTime[tp->mptcp->path_index - 1]);

        timeInQueue = timeInQueue; //microsecs same as RTT

        queueTime[tp->mptcp->path_index - 1] = timeInQueue;
        compBytes[tp->mptcp->path_index - 1] = completedBytes;
        sampleTime [tp->mptcp->path_index - 1] = currentTime;

        wait_time = timeInQueue;
    }else{
        /*We do not update the rate of completions if the 
         * queue size is 0. The wait time will be the 
         * previous known value of wait time in this case.
         */ 
        wait_time = queueTime[tp->mptcp->path_index -1];
    }

    //rtt = tp->srtt_us;
    //printk(KERN_INFO "MPTCP: The interface name is %s\n", dst->dev->name);
   
    //printk(KERN_INFO "New values for path index %u: PacketsInQueue: %u, QueueTime: %li, CompBytes: %u, DequeuedBytes: %u, SampleTime: %u \n\n", tp->mptcp->path_index, packets_in_queue, queueTime[tp->mptcp->path_index - 1], compBytes[tp->mptcp->path_index - 1], dequeuedBytes, sampleTime[tp->mptcp->path_index - 1]);
    ...
    mptcp_for_each_sk(mpcb, sk) {   //use path_index as id
		...
		if (unused) {
			if (!found_unused) {
				 ...
				 */
				//min_srtt = 0xffffffff;
                min_service = 0xffffffff;
                //set array values for this sk as zero
				bestsk = NULL;
			}
			found_unused = true;
		}
        ...
        /* switching function for service parameter
         */
				   ...
				   bool zero_wnd_test)
{
	...
	if (mpcb->cnt_subflows == 1) {
		sk = (struct sock *)mpcb->connection_list; // this is going to get the socket from the connection list of MPTCP
        //wait_time = get_wait_time(sk);
		if (!mptcp_is_available(sk, skb, zero_wnd_test)) // Check if given socket is available
			sk = NULL;
		return sk;
	}
	...
	sk = get_subflow_from_selectors(mpcb, skb, &subflow_is_active,
					zero_wnd_test, &force); // This function will return the subflow with better parameter, i.e. lower RTT
	if (force)
